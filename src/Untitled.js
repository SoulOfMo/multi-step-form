...
const updatePlan = function (plan, index) {
  ...
  return `<div><div class="plan-summary"><span class="plan-picked">$ {
    plan.planType[index]
  }

  </span><span class="plan-duration-summary">($ {
      plan.planName

    })</span></div><span class="change-plan">Change</span></div><div class="plan-picked-price">$$ {
    plan.planPrices[index]
  }
  ...
  const addOnsHtml = addsOnType
    .map(
      (
        type,
        id
      ) => ` <label for="${type}" > <div class="add-ons-type" > <input type="checkbox" name="${type}" id="${type}" value="${type}" > <div> <p>$ {
      type
    }

    </p> <span> $ {
      addsOnTypeDetails[id]
    }

    </span> </div> </div> <div class="add-ons-price" >+$$ {
      addsOnPrices[id]
    }
...
const updateAddOnsSummary = function (addOnPackage) {
  ...
  const addOnsResult = addonsSummary
    .map(function (result) {
      ...
      return `<div class="addOns-type" > <span class="addOn-type" >$ {
        result
      }
...
const updateTotalPrice = function () {
  ...
  totalElement.innerHTML = ` <span>Total (per $ {
      state.selectedPlan.type==="yearly" ? "year" : "month"

    })</span><span class="total-amount">$$ {
    total
  }
